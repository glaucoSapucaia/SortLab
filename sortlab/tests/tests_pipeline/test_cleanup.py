...
@pytest.fixture
def mock_cleanup():
    with mock.patch('sortlab.utils.delete_temp_files.clean_temp_files') as mock_clean:
        yield mock_clean

@pytest.fixture
def mock_logger():
    with mock.patch('logger.logger') as mock_log:
        yield mock_log

def test_cleanup_success(mock_cleanup, mock_logger):

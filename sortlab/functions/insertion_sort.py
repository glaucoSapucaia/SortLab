...
class InsertionSort(ISorter):
    ...
    def __init__(self, counter: IMetricCounter) -> None:
        """Inicializa o Insertion Sort com contador de métricas.
    ...
    def insertion_sort(self, arr: list[int]) -> list[int]:
        ...
        """
        try:
            logger.info(f"Iniciando InsertionSort para lista de tamanho {len(arr)}.")
            ...
            for j in range(1, len(arr)):
                ...
                while i >= 0 and arr[i] > key:
                    ...
                    self.counter.increase()  # Contabiliza movimentação
                    arr[i + 1] = arr[i]
                    i -= 1

                self.counter.increase()  # Contabiliza inserção final
                arr[i + 1] = key
